...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <small><?= get_phrase("caisse Gestionnaire"); ?></small>
      </h1>
        <ol class="breadcrumb">
            ...
            <li class="active"> <i class="glyphicon glyphicon-knight"></i>&nbsp;<?php echo get_phrase('Caisse externe'); ?></li>
        </ol>
    </section>
    <!-- Main content -->
        ...
        <div class="row">
            <div class="pull-left col-xs-12">
                <?php if(in_array('createCaisse', $this->permission)) {?>
                    ...
                    <a class="btn btn-success" href="<?php echo site_url();?>caisses/gexterne/refresh"><i class="fa fa-refresh" title="&nbsp;<?php echo get_phrase('restor'); ?>"></i></a>
            ...
            <div class="col-md-12 col-xs-12">
                ...
                <div class="col-md-6">
                    <div class="box box-warning collapsed-box" data-vivaldi-spatnav-clickable="1">
                        ...
                        <div class="box-body" style="display: none;">
                            ...
                            <h3 class="box-title text-right"><?php echo get_phrase('LAVAGE');?>: <b><?php echo$e_caisse1;?></b></h3>
                            <h3 class="box-title text-right"><?php echo get_phrase('KIOSQUE');?>: <b><?php echo$e_caisse2;?></b></h3>
                            <h3 class="box-title text-right"><?php echo get_phrase('AUTRE');?>: <b><?php echo$e_caisse3;?></b></h3>
                            <h3 class="box-title text-right"><b><?php echo$e_caisse;?></b></h3>
                        </div>
                ...
                <div class="col-md-6">
                    <div class="box box-warning collapsed-box" data-vivaldi-spatnav-clickable="1">
                        ...
                        <!-- /.box-header -->
                        <div class="box-body" style="display: none;">
                            <h3 class="box-title text-right"><?php echo get_phrase('LAVAGE');?>: <b><?php echo$s_caisse1;?></b></h3>
                            <h3 class="box-title text-right"><?php echo get_phrase('KIOSQUE');?>: <b><?php echo$s_caisse2;?></b></h3>
                            <h3 class="box-title text-right"><?php echo get_phrase('AUTRE');?>: <b><?php echo$s_caisse3;?></b></h3>
                            <h3 class="box-title text-right"><b><?php echo$s_caisse;?></b></h3>
            ...
            <div class="modal-body box box-success">
                <?php echo form_open('') ; ?>
                    <div class="box-body">
                        ...
                        <div class="form-group col-md-12" id="name_choix_type">
                        </div>
                        ...
                        <div class="input-group col-md-12" id="verifmontant">
...
<?php 
    ...
    //SORTIE DE CAISSE
        ...
        $postetype = '<select type="text" onclick="suite_type()" class="form-control" name="lier" id="lier" required ><option  value="">'.get_phrase('action pour').'</option><option  value="2">'.get_phrase('pour kiosque').'</option><option  value="1">'.get_phrase('pour lavage').'</option><option  value="3">'.get_phrase('pour autre').'</option></select>';
            ...
            $select3="<select onclick='autrename()' class='form-control col-md-6' name='nom_type' id='nom_tye' required ><option  >".get_phrase('Acte realise')."</option>";
            if($aff_type2){
                foreach($aff_type2 as $optionvalue){
                    if($optionvalue->liaison == 1){
                        $select3.="<option  value='";
                        $select3.=$optionvalue->id;
                        $select3.="'>";
                        $select3.=$optionvalue->nom;
                        $select3.="</option>";
                    }
                }
            }
            $select3.="<option value='0'>".get_phrase('Autre')."</option></select>";

            $select4="<select onclick='autrename()' class='form-control col-md-6' name='nom_type' id='nom_tye' required ><option  >".get_phrase('Acte realise')."</option>";
            if($aff_type2){
                foreach($aff_type2 as $optionvalue){
                    if($optionvalue->liaison == 3){
                        $select4.="<option  value='";
                        $select4.=$optionvalue->id;
                        $select4.="'>";
                        $select4.=$optionvalue->nom;
                        $select4.="</option>";
                    }
                }
            }
            $select4.="<option value='0'>".get_phrase('Autre')."</option></select>";
    ...
    //VALEUR DE AUTRE TYPE
        $valeurautre1="<input type='text' class='form-control' name='autre_nom_type' id='autre_valeur' disabled >";
    
    //info sur individu
        $content_tran="<input type='text' class='form-control' name='nom_indivi' placeholder='".get_phrase('Nom et prenom de la presonne ou de l entreprise')."' id='nom_indivi' required>";
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        manageTable = $('#manageTable').DataTable({
            ...
            'info'        : true,
            'autoWidth'   : false,
            'ajax': base_url + 'Caisses/fetchCaisseData/3',
            'order': []
            ...
            if (title=='<?php echo get_phrase('Realiser le'); ?>') {
              $(this).html( '<input type="date" placeholder=" Recherche" style="width:100%"/>' );
                ...
                if (title=='<?php echo get_phrase('Mouvement'); ?>') {
                    $(this).html( '<input type="text" id="'+title+'" placeholder="<?php echo get_phrase('search'); ?>" list="mouv" /><datalist id="mouv"><option ><?php echo get_phrase('entrer en caisse'); ?></option><option ><?php echo get_phrase('sortie de caisse'); ?></option><option ><?php echo get_phrase('memberships'); ?></option><option><?php echo get_phrase('memberships_fee'); ?></option><option ><?php echo get_phrase('deleted'); ?></option></datalist>' );
    ...
    function info_type(){
        ...
        if (valeur=="") {
            ...
            document.getElementById('name_choix_type').innerHTML="";
            ...
            if (valeur==2) {
                document.getElementById('name_choix_type').innerHTML='<?=$postetype;?>';
              document.getElementById('personne').innerHTML="";
              document.getElementById('name_new_type').innerHTML="";
    ...
    function suite_type(){
        var select = document.getElementById("lier");
        var valeur = select.options[select.selectedIndex].value;
        if (valeur=="") {
            document.getElementById('name_new_type').innerHTML="";
            document.getElementById('personne').innerHTML="";
            ...
            if (valeur==2) {
                document.getElementById('personne').innerHTML="<?= $select3;?>";
              document.getElementById('name_new_type').innerHTML="<?= $valeurautre1;?>";
            }
            if (valeur==3) {
                document.getElementById('personne').innerHTML="<?= $select4;?>";
              document.getElementById('name_new_type').innerHTML="<?= $valeurautre1;?>";
            }
    ...
    function suite_type2(){
        var select = document.getElementById("lierupdate");
        var valeur = select.options[select.selectedIndex].value;
        if (valeur=="") {
            document.getElementById('name_new_type_update').innerHTML="";
            document.getElementById('personne_update').innerHTML="";
        }else{
            
            if (valeur==1) {
                document.getElementById('personne_update').innerHTML="<?= $select2;?>";
                document.getElementById('name_new_type_update').innerHTML="<?= $valeurautre1;?>";
            }
            if (valeur==2) {
                document.getElementById('personne_update').innerHTML="<?= $select3;?>";
              document.getElementById('name_new_type_update').innerHTML="<?= $valeurautre1;?>";
            }
            if (valeur==3) {
                document.getElementById('personne_update').innerHTML="<?= $select4;?>";
              document.getElementById('name_new_type_update').innerHTML="<?= $valeurautre1;?>";
            }
    ...
    function autrename3(){
        var select = document.getElementById("nom_tye_update");
        var valeur = select.options[select.selectedIndex].value;
        if (valeur== 0) {
            document.getElementById('autre_valeur2_update').required=true;
            document.getElementById('autre_valeur2_update').disabled=false;
        }else{
            document.getElementById('autre_valeur2_update').required=false;
            document.getElementById('autre_valeur2_update').disabled=true;
