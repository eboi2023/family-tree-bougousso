    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <?php 
          if(isset($aff_recup)){
            ?>
            <li><a href="<?php echo site_url('rapport');?>"><?= $icon; ?>&nbsp;<?php echo get_phrase($lien); ?></a></li>
            <li class="active"><i class="fa fa-plug"></i> &nbsp;<?= get_phrase('update rapport'); ?></li>
            <?php
          }else{
            ?>
                ...
                <div class="col-md-12" id="modifreport">
                    <div class="box box-info">
                      <div class="box-header">
                        <input type="submit" id="valider1" value="<?php if (isset($aff_recup)) {
                          echo get_phrase('updated');
                        } else {
                          echo get_phrase('Enregistrer');
                        }
                        ?>" class="btn btn-success" name="envoi" onclick="recharge()" />
                        <h3 class="box-title">
                          <small></small>
                        </h3>
                        <!-- tools box -->
                    ...
                    <div class="box-body pad">
                        <div class="form-group col-xs-9">
                            ...
                            <input type="text" class="form-control" name="titre" value="<?php if(isset($aff_recup)){echo $aff_recup['titre'];}?>" id="titre_transmiserapport" placeholder="<?php echo get_phrase('Le titre du rapport'); ?>" required>
                        </div>
                        <div class="form-group col-xs-3">
                            <label for=""><?php echo get_phrase('Date du rapport'); ?></label>
                            <input type="date" class="form-control" name="date_titre" value="<?php if(isset($aff_recup)){echo date('Y-m-d',$aff_recup['date_rapport']);}else{echo date('Y').'-'.date('m').'-'.date('d');}?>" id="date_transmiserapport" placeholder="<?php echo get_phrase('Le titre du rapport'); ?>" max="<?php echo date('Y').'-'.date('m').'-'.date('d'); ?>" required>
                        </div>
                        <div class="form-group col-xs-12">
                            <textarea id="editor1" class="form-control summernote" name="editor1" rows="10" cols="80" ><?php if(isset($aff_recup)){echo $aff_recup['contenu'];}?></textarea>
                        </div>
                      ...
                      <div class="form-group col-xs-12">
                        ...
                        <div class="form-group">
                          ...
                          <?php if(isset($aff_recup)){echo '<input type="hidden" name="verifcode" value="'.$aff_recup['id'].'">';} ?>
                        </div>
                        <input type="submit" id="valider2" value="<?php if (isset($aff_recup)) {
                          echo get_phrase('updated');
                        } else {
                          echo get_phrase('Enregistrer');
                        }
...
<div class="modal modal-default fade" id="modal-valider">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">
                    <center>
                        <h2 class="box-title"><?php echo get_phrase('Aviez-vous lu pour passer a la validation?'); ?></h2>
                        <h4><?php echo get_phrase('Si oui entrer le code de validation et cliquer sur valider'); ?></h4>
                    </center>
                </h4>
            </div>
            <div class="modal-body box box-success">
                ...
                ?>
                    <div class="box-body">                        
                        ...
                        <input type="hidden" name="codeverifinconf" value="valideRapport">
                        <input type="hidden" name="verifcode" id="verifcode2">

                    </div>
                    <div class="modal-footer box box-success">
                        <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Fermer</button>
                        <input type="submit" id="valider4" value="<?php echo get_phrase('valider'); ?>" class="btn btn-success" name="envoi" />
...
<script type="text/javascript">
    ...
    function valider_rapport(m){
      document.getElementById('verifcode2').value=m;
    }
    function datetranslet(valeur){ 
      $.ajax({
        url: base_url+'Evenement/convertdate/'+valeur,
        type: 'GET',
        dataType: 'json',
        success:function(response) {
            document.getElementById('date_transmiserapport').value=response;
        }
      });
    }
    function modif_mouv(pp){
      $.ajax({
        url: base_url+'Rapport/updateRapportById/'+pp,
        type: 'GET',
        dataType: 'json',
        success:function(response) {
          $.each(response, function(index, values) {
            document.getElementById('titre_transmiserapport').value=values.titre;
            datetranslet(values.date_rapport);
            document.getElementById('editor1').innerHTML=values.contenu;
            document.getElementById('valider1').value="<?php echo get_phrase('updated'); ?>";
